            ...
            pollRef.current = setInterval(async () => {
                ...
                try {
                    ...
                    if (!check.data.success) {
                        console.error("Polling error:", check.data.message);
                        return;
                    }
                    const analysis = check.data.data;

                    console.log("Polling analysis status:", analysis.status, "attempt:", attempts);
                    ...
                    if (analysis.status === "completed") {
                        ...
                        if (pollRef.current) clearInterval(pollRef.current);
                        const errorMsg = analysis.issues && analysis.issues.length > 0 
                            ? analysis.issues[0].recommendation 
                            : "Analysis failed. Please try again.";
                        setError(errorMsg);
                        setAnalyzing(false);
                    } else {
                        // still processing - advance visual steps based on progress
                        if (attempts === 3) setCurrentStep(1); // After 6 seconds, show scanning
                        if (attempts === 10) setCurrentStep(2); // After 20 seconds, show AI analysis
